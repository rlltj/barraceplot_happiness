...
class barplot(object):
    ...
    * value_column: numerical value
    ...
    def __init__(self, df: pd.DataFrame = None, item_column: str = None, value_column: str = None , category_column: str = None, rank_column: str = None, time_column: str = None , item_color: str = None , top_entries: int = 10):
        self.df = df
    ...
    def __make_frames(self, title: str, frame_duration: int):
        ...
        x, y, z, r = self.__check_orientation()
        frames = []
        ...
        for date in dates:
            ...
            rt = snap_data[r].astype(str)
            snap_data['text'] = rt + " - " + snap_data[y].astype(str) + ", " + snap_data[z]
                ...
                go.Frame(
                    data=[
                        go.Bar(
                            ...
                            hovertemplate=
                            "<b>%{text}</b><br>" +
                            "Happiness Score: %{x}<br>" +
                            "<extra></extra>", #annoying and obscure, see docs
                            ## (https://community.plotly.com/t/remove-trace-0-next-to-hover/33731)
                            textposition='inside',
                            texttemplate='%{x}' if self.orientation == 'vertical' else '%{y}',
                            textangle = 0,
                    ...
                    layout=go.Layout(
                        ...
                        } if self.orientation == 'vertical' else
                        {
                            'showline': True,
                            'visible': True,
                            'range': (self.df[self.value_column].min()-0.25, self.df[self.value_column].max())
                        },
                        yaxis={
                            'showline': True,
                            'visible': True,
    ...
    def __make_frame1(self, time_frame1 = 'min'):
        ...
        x, y, z, r = self.__check_orientation()
        rt = frame1[r].astype(str)
        frame1['text'] = rt + " - " + frame1[y].astype(str) + ", " +frame1[z]
    ...
    def __get_colors(self):
        if (self.item_color == None):
            colors = {item: 'rgb({}, {}, {})'.format(*sample(range(256), 3)) for item in self.df[self.category_column].unique()}
            self.df['color'] = self.df[self.category_column].map(colors)
            ...
            self.df['color'] = self.df[self.category_column].map(self.item_color)
        else:
